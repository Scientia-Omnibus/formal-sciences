...
y =  \frac{2}{3}  Then, we substitute this value for y into one of the original equations and solve for x.
...
x = - \frac{7}{3}  The solution to this system is ( - \frac{7}{3} ,  \frac{2}{3}  )  Check the solution in the first equation.

x + 2y = -1

( - \frac{7}{3}  ) + 2(  \frac{2}{3}  ) = -1
...
y = -  \frac{1}{2} x +  \frac{9}{2}  We then convert the second equation expressed to slope-intercept form.

x + 2y = 13

2y = - x + 13

y = - \frac{1}{2} x +  13 ___ 2  Comparing the equations, we see that they have the same slope but different y-intercepts. Therefore, the lines are parallel and do not intersect.

y = - \frac{1}{2} x +  \frac{9}{2} 

y = - \frac{1}{2} x +  13 ___ 2  Analysis Writing the equations in slope-intercept form confirms that the system is inconsistent because all lines will intersect eventually unless they are parallel. Parallel lines will never intersect; thus, the two lines have no points in common. The graphs of the equations in this example are shown in Figure 8. x y y = -x + y = -x +
...
y = - \frac{1}{3} x +  \frac{2}{3}  See Figure 9. Notice the results are the same. The general solution to the system is ( x, - \frac{1}{3} x +  \frac{2}{3}  ) . x y x + 3y = 2 3x + 9y = 6
...
Let c = the number of children and a = the number of adults in attendance. The total number of people is 2,000. We can use this to write an equation for the number of people at the circus that day.
...
of a System of Dependent Equations Containing Three Variables We know from working with systems of equations in two variables that a dependent system of equations has an infinite number of solutions. The same is true for dependent systems of equations in three variables. An infinite number of solutions can result from several situations. The three planes could be the same, so that a solution to one equation will be the solution to the other two equations. All three equations could be different but they intersect on a line, which has infinite solutions. Or two of the equations could be the same and intersect the third on a line.
...
z =  \frac{3}{2} x We back-substitute the expression for z into one of the equations and solve for y.
